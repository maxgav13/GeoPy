...
```{python}
data = pd.read_table("GeoPy/PyGimli/VES/curso_geofisica/datos/SEV1.TXT")
print(data.head())
vescode = "SEV-1" # Código del sondeo, nombre a usar en archivos de salida

# ab2, mn, roa = np.genfromtxt("GeoPy/PyGimli/VES/curso_geofisica/datos/SEV1.TXT", 
#                              skip_header=1,
#                              unpack=True)
...
Si existen valores atípicos en los datos, se pueden eliminar. En este caso, el valor con índice 21 es un valor atípico evidente, ver gráfico de datos más abajo. Además, reiniciamos los índices del DataFrame.
...
# plt.savefig("GeoPy/PyGimli/VES/" + vescode + ".png", dpi=300, bbox_inches='tight')
```
...
fig, ax = plt.subplots(figsize=(4,5))
vesM.showFit(ax=ax)
# drawModel1D(model=modelM, ax=ax, label="Modelo", plot='loglog')
# ax.set_ylim(max(ab2)*1.1, min(ab2)*0.9)
...
## Inversión con diferentes métodos
...
# plt.savefig("GeoPy/PyGimli/VES/" + vescode + "-chi2-marquardt-.png", dpi=300, bbox_inches='tight')
```
...
# plt.savefig("GeoPy/PyGimli/VES/" + vescode + "-chi2-vesmanager-.png", dpi=300, bbox_inches='tight')
```
...
ax.loglog(ab2, ves.inv.response, "-", color='green', label="Bloques1D")
ax.legend()
ax.grid(True, which='both')
ax.set_ylabel(r"Resistividad aparente $\rho_a$ ($\Omega$m)")
ax.set_xlabel("AB/2 (m)")
...
drawModel1D(model=model2, ax=ax, color='green', label="Bloques1D", plot='semilogx')
...
# plt.savefig("GeoPy/PyGimli/VES/" + vescode + "-fit-modelos-" + str(ncapas) + "capas.png", dpi=300, bbox_inches='tight')
```
...
```{python}
Inv = inv # inv o ves.inv
Model = model # model o model2
...
with open("GeoPy/PyGimli/VES/" + vescode + "-modelo-" + str(ncapas) + "capas.txt", "w") as f:
...
# drawModel1D(model=Model, ax=ax[0], color='orange', label="modelo", plot='loglog')
# ax[0].set_ylim(max(ab2)*1.1, min(ab2)*0.9)
ax[0].invert_yaxis()
ax[0].legend()
...
# plt.savefig("GeoPy/PyGimli/VES/" + vescode + "-fit-" + str(ncapas) + "capas.png", dpi=300, bbox_inches='tight')
```
...
# plt.savefig("GeoPy/PyGimli/VES/" + vescode + "-correl-" + str(ncapas) + "capas.png", dpi=300, bbox_inches='tight')
```
...
# plt.savefig("GeoPy/PyGimli/VES/" + vescode + "-fit-errorbars-" + str(ncapas) + "capas.png", dpi=300, bbox_inches='tight')
```
