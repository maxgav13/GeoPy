...
```{python}
data = ert.load('GeoPy/PyGimli/ERT/Wenner_1_2.dat', verbose=True)
print(data)
tomocode = "Wenner-1" # Código del perfil, nombre a usar en archivos de salida
```
...
```{python}
topo = np.loadtxt('GeoPy/PyGimli/ERT/DD_topo.txt', skiprows=1)
```
...
```{python}
ert.showData(data, vals=data['rhoa'])
```
...
# plt.savefig('GeoPy/PyGimli/ERT/ert-' + tomocode + '-chi2-iter.png', dpi=300, bbox_inches='tight')
```
...
with open("GeoPy/PyGimli/ERT/resistividades-" + tomocode + "-.txt", "w") as f:
    f.writelines(rlist)
```
...
# plt.savefig('GeoPy/PyGimli/ERT/tomo-res-fit.png', dpi=300, bbox_inches='tight')
```

```{python}
mgr_ert.showFit()

# plt.savefig('GeoPy/PyGimli/ERT/tomo-fit.png', dpi=300, bbox_inches='tight')
```

```{python}
mgr_ert.showMisfit()

# plt.savefig('GeoPy/PyGimli/ERT/tomo-misfit.png', dpi=300, bbox_inches='tight')
```
...
ax, cb = mgr_ert.showResult(**kw_ert, orientation='horizontal')
...
# plt.savefig('GeoPy/PyGimli/ERT/tomo-res-' + tomocode + '-.png', dpi=300, bbox_inches='tight')
```
...
# plt.savefig('GeoPy/PyGimli/ERT/tomo-mesh-' + tomocode + '-.png', dpi=300, bbox_inches='tight')
```
...
# plt.savefig('GeoPy/PyGimli/ERT/res-model-fit-' + tomocode + '-.png', dpi=300, bbox_inches='tight')
```
...
# plt.savefig('GeoPy/PyGimli/ERT/tomo-res-contour-' + tomocode + '-.png', dpi=300, bbox_inches='tight')
```
...
```{python}
ax, cb = pg.show(result, result["cond"], cMap="RdYlBu", logScale=True, nLevs=10, figsize=(8,4))
ax.set_ylabel('Z (m)')
ax.set_xlabel('Distancia (m)')
ax.set_aspect(aspect=1)
ax.set_frame_on(False)
...
# plt.savefig('GeoPy/PyGimli/ERT/tomo-cond-' + tomocode + '-.png', dpi=300, bbox_inches='tight')
```
...
# plt.savefig('GeoPy/PyGimli/ERT/tomo-res-' + tomocode + '-1D-' + f"{x[0]:.0f} m" + ".png", dpi=300, bbox_inches='tight')
```
...
```{python}
mgr_ert.saveResult('GeoPy/PyGimli/ERT/tomo-res-' + tomocode + '/')
```

Salva los datos en un formato unificado

```{python}
data.save('GeoPy/PyGimli/ERT/Wenner_py.dat')
```