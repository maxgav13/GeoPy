...
Vemos el modelo y su ajuste con los datos. La línea de código comentada permite comparar con el modelo manual definido anteriormente.

```{python}
fig, ax = plt.subplots(figsize=(4,5))
drawModel1D(ax, espesores, res_r, color='blue', label="RhoMod", plot='semilogx')
# drawModel1D(model=modelM, ax=ax, color='orange', label="Manual", plot='semilogx')
ax.set_xlabel(r"Resistividad $\rho$ ($\Omega$m)")
ax.set_ylabel("Profundidad (m)")
ax.grid(True, which='both')
ax.legend(loc='best')
